...
{
  int len = snprintf(pub_topic, BUFFER_SIZE, "topic 0");
...
subscribe()
{
  ...
  int rand_num = rand() % 2;
  snprintf(sub_topic, BUFFER_SIZE, "topic %d", rand_num);
...
publish(void)
{
  ...
  int rand_num = rand() % 2;
  snprintf(pub_topic, BUFFER_SIZE, "topic %d", rand_num);

  int rc_publish = mqtt_publish(&conn, NULL, pub_topic, (uint8_t *)app_buffer,
               strlen(app_buffer), MQTT_QOS_LEVEL_0, MQTT_RETAIN_OFF);

  if(rc_publish == MQTT_STATUS_OK){
      LOG_INFO("Publish sent out on topic '%s'!\n", pub_topic);
  }else{
      LOG_INFO("Publish failed on topic '%s'!\n", pub_topic);
  }
}
/*---------------------------------------------------------------------------*/
static void
